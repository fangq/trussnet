...
bool TetMesh::getTetsFromFaceVertices(uint32_t v1, uint32_t v2, uint32_t v3, uint64_t* nt) const {
    ...
    VTfull(v1, vt);
    int i = 0;
    // gpu_brain2mesh: bound the writes -- nt has room for 2, but a degenerate cavity
    // DT can have more tets on these vertices (overflowed the caller's stack array:
    // "stack smashing" / segfault on real-head PLCs). More than 2 -> false, which
    // the callers treat like a missing face (expand the cavity).
    for (uint64_t t : vt) if (tetHasVertex(t, v2) && tetHasVertex(t, v3)) { if (i < 2) nt[i] = t; ++i; }
    vt.clear();
    return (i == 2);
}
