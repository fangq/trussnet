...
bool PLCx::recoverFaceHSi(std::vector<uint64_t>& i_tets, const PLCface& f, bool& sisMethodWorks) {
    ...
    // gpu_brain2mesh: a cavity that would expand into a GHOST tet (the convex hull;
    // the assert in expandCavity is compiled out in release builds, and the ghost's
    // infinite vertex then indexes v_orient out of range) is treated like a failed
    // Si cavity -> the gift-wrap fallback below.
    while ((t = missingFaceInCavity(top_faces, top_vertices))!=UINT64_MAX) {
        if (delmesh.isGhost(t >> 2)) { cavity_ok = false; break; }
    ...
    while (cavity_ok && (t = missingFaceInCavity(bottom_faces, bottom_vertices)) != UINT64_MAX) {
        if (delmesh.isGhost(t >> 2)) { cavity_ok = false; break; }
