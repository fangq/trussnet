...
#include <bit>   // std::countl_zero (C++20); a C++17 fallback is used otherwise
#endif
#include <bitset>
#include <cstdint>
...
#if defined(__cpp_lib_bitops)   // std::countl_zero (C++20)
		auto cz = std::countl_zero(bs - 1);
		cz = (cz == 32) ? (31) : (cz);
		return IN_pools[31 - cz];
#else                           // C++17 fallback (the author's, see below)
