...
// gpu_brain2mesh: throw instead of exit(0). exit() silently terminated the whole
// host process -- with a SUCCESS status -- on any CDT failure, which would kill a
// MATLAB/Octave/Python session; the caller now gets a catchable exception.
inline void ip_error(const char* msg)
{
	std::cerr << msg;
	throw std::runtime_error(std::string("cdt: ") + msg);
}
